...
def send_sql_command(func, keys, datas):
    ...
    return _send_command_to_db_server(sql_command)


def _send_command_to_db_server(sql_command):
    response = None
    ...
    return _db_server_response_handler(response)


def _db_server_response_handler(db_server_response):
    try:
